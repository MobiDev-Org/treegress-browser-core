...
import type { AICustomDomFrameData } from '@isomorphic/aiSnapshotTypes';
...
import { captureCustomDomSnapshot } from './customDomSnapshot';
...
export class InjectedScript {
  ...
  customDomSnapshot(node: Node): AICustomDomFrameData {
    if (node.nodeType !== Node.ELEMENT_NODE)
      throw this.createStacklessError('Can only capture custom DOM snapshot of Element nodes.');
    return captureCustomDomSnapshot(node as Element);
  }
