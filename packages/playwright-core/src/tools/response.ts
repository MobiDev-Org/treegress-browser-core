...
export class Response {
  ...
  private _snapshotBackend?: 'aria' | 'custom-dom';

  setSnapshotBackend(backend?: 'aria' | 'custom-dom') {
  ...
  private async _build(): Promise<Section[]> {
    ...
    const tabSnapshot = this._context.currentTab()
      ? await this._context.currentTabOrDie().captureSnapshot(this._clientWorkspace, this._snapshotBackend)
      : undefined;
