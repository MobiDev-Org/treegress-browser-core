...
import { compileLocatorCandidatesFromPlan, inspectLocatorCandidates } from './customDomLocatorCompiler';
import { CustomDomResolverError, type CustomDomResolverDiagnostics } from './customDomResolverDiagnostics';
import { customDomStableLocatorKey, formatCustomDomSnapshot, isCustomDomSnapshotEnvelope } from './customDomSnapshotFormatter';
...
import type { CustomDomSnapshotRefAlias } from './customDomSnapshotFormatter';

const TabEvents = {
  modalState: 'modalState'
};

const customDomDebug = debug('pw:mcp:custom-dom');
...
export class Tab extends EventEmitter<TabEventsInterface> {
  ...
  private _customDomAliases = new Map<string, CustomDomSnapshotRefAlias>();
  private _customDomLocatorPlans = new Map<string, AICustomDomLocatorPlan>();
  ...
  private _clearCustomDomSnapshotCache() {
  ...
  async captureSnapshot(relativeTo: string | undefined, snapshotBackend?: 'aria' | 'custom-dom'): Promise<TabSnapshot> {
    await this._initializedPromise;
    ...
    const modalStates = await this._raceAgainstModalStates(async () => {
      ...
      const snapshot = await this.page._snapshotForAI({
        track: 'response',
        backend: snapshotBackend ?? 'custom-dom',
      });
      if (snapshot.backend === 'custom-dom' && isCustomDomSnapshotEnvelope(snapshot.envelope)) {
  ...
  async refLocators(params: { element?: string, ref: string }[]): Promise<{ locator: Locator, resolved: string }[]> {
    await this._initializedPromise;
    const strictCustomDom = true;
    const customDomSnapshotSeen = this._customDomAliases.size > 0;
    return Promise.all(params.map(async param => {
      try {
        const customDomResolved = await this._resolveCustomDomRef(param);
        if (customDomResolved.locator && customDomResolved.resolved) {
          customDomDebug(`resolved ref=${param.ref} via custom-dom`);
          return { locator: customDomResolved.locator, resolved: customDomResolved.resolved };
        }
        if (strictCustomDom || customDomSnapshotSeen) {
          throw new CustomDomResolverError(customDomResolved.diagnostics);
        }
        let locator = this.page.locator(`aria-ref=${param.ref}`);
        if (param.element)
          locator = locator.describe(param.element);
        customDomDebug(`resolved ref=${param.ref} via aria fallback`);
        return { locator, resolved: await locator._resolveForCode() };
      } catch (e) {
        if (e instanceof CustomDomResolverError)
          throw e;
  ...
  private _resolveFrameByPath(framePath: number[]): Frame | undefined {
    let frame = this.page.mainFrame() as Frame;
    for (const childIndex of framePath) {
      const childFrames = frame.childFrames() as Frame[];
  ...
  private async _resolveCustomDomRef(param: { element?: string, ref: string }): Promise<{ locator?: Locator, resolved?: string, diagnostics: CustomDomResolverDiagnostics }> {
    const diagnostics: CustomDomResolverDiagnostics = {
      ref: param.ref,
      alias: {
        status: 'missing',
      },
    ...
    const alias = this._customDomAliases.get(param.ref);
    if (!alias)
      return { diagnostics };
    ...
    const planKey = customDomStableLocatorKey(alias.framePath, alias.stableId);
    ...
    const candidates = compileLocatorCandidatesFromPlan(frame, locatorPlan);
    ...
    const inspection = await inspectLocatorCandidates(candidates);
    ...
    customDomDebug(
        `ref=${param.ref} stableId=${alias.stableId} framePath=${alias.framePath.join('.') || 'root'} ` +
        `strategy=${candidate.strategy} selector=${candidate.locator._selector}`);
